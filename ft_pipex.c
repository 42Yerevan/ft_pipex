...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/31 16:25:41 by vmusaely          #+#    #+#             */
/*   Updated: 2021/07/31 16:25:43 by vmusaely         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pipex(int argc, char **argv)
{
	...
	if ((out = open(argv[argc - 1], O_WRONLY | O_CREAT | O_TRUNC, 0777)) == -1)
...
void	ft_pipein(int pipefd[2], int in, char *cmd1)
{
	...
	args = ft_split(cmd1, ' ');
	if (dup2(in, 0) == -1)
		ft_exit("Failed to copy input file descriptor");
	if (dup2(pipefd[1], 1) == -1)
		ft_exit("Failed to connect STDOUT");
	if (args[0][0] == '/')
	{
